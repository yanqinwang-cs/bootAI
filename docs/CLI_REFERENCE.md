...
Stages 11.2 through 11.4 use a separate module launcher; they do not add or change an `organizer.cli` flag:
...
The current web application provides an authenticated read-only scan dashboard and review explorer. It does not expose review decisions, apply, verification, undo, or restore actions. All `organizer.cli` behavior below is unchanged.
